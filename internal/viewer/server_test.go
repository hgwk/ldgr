...
func TestServer_CanonicalKanbanShape(t *testing.T) {
	dir := t.TempDir()
	cfg := config.Default("myapp", "id-canonical", "")
	cfg.SchemaVersion = 1
	if err := os.MkdirAll(filepath.Join(dir, "ledger"), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	...
	os.WriteFile(filepath.Join(dir, "ledger", "tickets.jsonl"),
		[]byte(`{"n":1,"ts":"2026-05-14T10:00:00Z","id":"CANON-1","parent":"ROOT","type":"task","state":"ready","area":"backend","priority":"P1","title":"t","owner":"codex","blocked_by":[],"acceptance":[],"evidence":[],"event":{"actor":"codex","role":"planner","summary":"opened","notes":""}}`+"\n"), 0o644)
	os.WriteFile(filepath.Join(dir, "ledger", "worklog.jsonl"), []byte{}, 0o644)
	srv, err := NewSingleProjectServer(dir)
	if err != nil {
		t.Fatalf("server: %v", err)
	...
	var resp map[string]any
	if c := getJSON(t, srv.Handler(), "/api/projects/id-canonical/kanban", &resp); c != 200 {
		t.Fatalf("status %d", c)
	}
	cols, _ := resp["columns"].([]any)
	if len(cols) != 8 {
		t.Fatalf("want 8 canonical columns, got %d", len(cols))
	}
	grid, _ := resp["grid"].([]any)
	if len(grid) != 2 {
		t.Fatalf("want 2-row grid, got %+v", resp["grid"])
