...
- **모듈**: `github.com/hgwk/ldgr`
- **언어**: Go 1.22+, 표준 라이브러리만 (외부 의존성 0개 목표)
- **작성일**: 2026-05-14
- **상태**: v0 설계 기준 + schema v1 migration draft
...
- 모든 쓰기와 검증은 단일 Go 바이너리(`ldgr`)를 통한다.
- 데이터는 레포 안 JSONL/JSON 파일에 저장되어 git이 자동으로 버전·동기화를 담당한다.

## 2. 배포

- 1차: GitHub Releases prebuilt binary (`darwin-arm64`, `darwin-amd64`, `linux-amd64`, `linux-arm64`) + `go install github.com/hgwk/ldgr@latest`
- 2차: Homebrew tap (후순위)
...
| 프로젝트 레지스트리 | 레포 밖 `~/.ldgr/registry.json` |
...
<repo>/
  ledger/
    ...
    instructions/
      AGENTS.ldgr.md
      CLAUDE.ldgr.md
  AGENTS.md             # LDGR 블록 prepend (옵션)
  CLAUDE.md             # LDGR 블록 prepend (옵션)
```
...
- `writing_language`: LLM/agent가 ledger 자유 텍스트 필드(`task`, `notes`, `result`, `decision`, `audit_notes`, `handoff`, `summary`, `acceptance`)를 작성할 언어 코드. 빈 문자열은 지정 없음이다. 스키마 필드명, enum 값, code identifier, file path, command line은 번역하지 않는다.
...
- `goal set --log` 또는 `log_goal_changes: true`가 만드는 자동 worklog는 ticket 없는 행으로 기록한다. 기본 형태:
  ```json
  {
    ...
    "commands": ["ldgr goal set --json @- --log"],
    "notes": ""
  }
  ```
...
```json
{
  ...
  "acceptance": ["go test ./...", "ldgr verify"],
  "blocked_by": [],
  "decision": "",
  "notes": "",
  "lead": "codex",
...
ldgr의 기본 delivery 흐름은 **Plan → Develop → Audit → Complete**다. 감리는 새 원장을 만들지 않고 `tickets.jsonl`의 상태 전이 row로 기록한다. `worklog.jsonl`은 여전히 완료 산출물 전용이다.
...
```json
{
  ...
  "evidence": ["go test ./...", "ldgr import legacy --plan"]
}
```
...
저장 모델에서 `ticket`은 legacy/internal automation 호환을 위해 optional이다(예: `goal set --log`로 자동 생성된 행은 ticket 없이 기록 가능). 그러나 사람/에이전트가 직접 쓰는 `ldgr worklog add`는 `ticket`을 필수로 요구한다. 그 외 필드는 필수.
...
### 3.5B Ledger schema v1 (migration target)

`schema_version=1` is the canonical shipped schema. Historical `ticket/status/task` rows are treated as legacy rows and are converted by explicit migration, not mixed gradually.

Schema v1의 목적:

- `status/kind/category/role/task`처럼 책임이 겹치는 필드를 제거한다.
- ticket lifecycle은 `state`, ticket shape은 `type`, work domain은 `area`, row 작성 역할은 `event.role`로 분리한다.
- Kanban은 `state`만으로 4×2 grid를 만든다.
- 기존 프로젝트는 자동 변환하지 않는다. `ldgr migrate legacy-to-v1 --plan|--apply`가 유일한 전환 경로다.

#### 3.5B.1 v1 `config.json`
...
```json
{
  ...
  "name": "My App",
  "parents": ["ROOT", "DOC", "FE", "BE", "OPS", "DEMO", "BUG", "LEGACY"],
  "branch_convention": "work/{id}",
  "log_goal_changes": false,
  "writing_language": ""
}
```

#### 3.5B.2 v1 `tickets.jsonl` row

`tickets.jsonl` remains the append-only ticket event log. Each row is the latest ticket snapshot plus event metadata.
...
Required v1 ticket fields:

- top-level: `n`, `ts`, `id`, `parent`, `type`, `state`, `area`, `priority`, `title`, `owner`, `blocked_by`, `acceptance`, `evidence`, `event`
- event object: `actor`, `role`, `summary`, `notes`
...
| `event.result` | optional: `pass`, `changes_requested`, `cancelled`, `corrected` |
...
- `state=done` requires `event.role=auditor`, `event.result=pass`, positive `event.reviewed_n`, and non-empty top-level `evidence`.
- `state=rework` requires `event.role=auditor`, `event.result=changes_requested`, positive `event.reviewed_n`, and non-empty `event.notes`.
- Implementers move work to `state=review`; auditors move it to `done` or `rework`.

#### 3.5B.3 v1 `worklog.jsonl` row

Worklog stays as the completed-delivery ledger. It no longer carries lifecycle state.
...
- `ticket`, `actor`, `title`, `summary`, `paths`, `commands`, and `notes` are required.
- `ticket` must reference a ticket whose latest v1 state is `done`.
- `goal set --log` may still write an internal ticket-less worklog only if explicitly documented as ldgr-owned automation; human/agent `worklog add` requires `ticket`.

#### 3.5B.4 v1 Kanban grid

Schema v1 Kanban is a state grid, not a role/category projection:
...
```json
{
  "grid": [
    [
      { "id": "ready", "title": "Ready", "tickets": [] },
      { "id": "doing", "title": "Doing", "tickets": [] },
      { "id": "review", "title": "Review", "tickets": [] },
      { "id": "done", "title": "Done", "tickets": [] }
    ],
    [
      { "id": "backlog", "title": "Backlog", "tickets": [] },
      { "id": "blocked", "title": "Blocked", "tickets": [] },
      { "id": "rework", "title": "Rework", "tickets": [] },
      { "id": "dropped", "title": "Dropped", "tickets": [] }
    ]
  ]
}
```

The viewer keeps the legacy `columns` projection only for unmigrated legacy rows.

#### 3.5B.5 legacy-to-v1 migration

Migration is an explicit, atomic rewrite operation and is the documented exception to runtime append-only writes.
...
- `--plan`: prints row counts, field mappings, defaulted values, unmapped fields, and warnings. It writes nothing.
- `--apply`: writes a backup under `ledger/.backup/legacy-to-v1-<ts>/`, rewrites `config.json`, `tickets.jsonl`, and `worklog.jsonl` to schema v1, and leaves `goal.json` unchanged except for compatible formatting if required.
- Migration output must pass `ldgr verify`.
- Existing rows are not patched in place during normal operation. Only the migration command may rewrite files.
...
- `kind` present: copy to `type`.
- missing `kind` + `role=plan`: `type=plan`.
- missing `kind` + `role=audit`: `type=audit`.
- otherwise: `type=task`.

Area defaulting:

- `category` present: map to `area` when it is a v1 area value.
- `category=docs` or parent `DOC`: `area=docs`.
- `category=test`: `area=test`.
- `category=infra`: `area=infra`.
- `category=release`: `area=release`.
- otherwise default `area=ops` and emit `AREA_DEFAULTED`.

Role mapping:

| v1 role | v1 `event.role` |
|---|---|
| `impl` | `implementer` |
| `plan` | `planner` |
| `audit` | `auditor` |
...
### 3.6 동시성: `ledger/.lock` / `~/.ldgr/registry.lock`

단일 사용자라도 Codex / Claude / pre-commit 훅 / 사람 터미널이 동시에 append할 수 있다. `n` 무결성을 지키기 위해 **모든 쓰기 경로**는 다음 락 프로토콜을 따른다. 레지스트리(`~/.ldgr/registry.json`)도 동일 프로토콜로 `~/.ldgr/registry.lock`을 사용한다 — `init`/`list --prune`/`registry repair` 동시 실행 충돌 방지.
...
- `ldgr doctor`(후순위)에서 stale lock 진단·정리 옵션 제공.
...
```
ldgr init [--target PATH] [--name NAME] [--slug SLUG]
          [--language ko|en|...]
ldgr view [--port 3030]
ldgr list [--prune]
ldgr unregister (--project-id ID | --path PATH)
ldgr registry repair
ldgr hooks install
ldgr hooks uninstall
ldgr instructions install [--target PATH]
ldgr instructions uninstall [--target PATH] [--keep-bodies]
ldgr verify [--target PATH] [--strict]
ldgr next --ticket ID [--target PATH] [--format text|json]
ldgr suggest worklog --ticket ID [--target PATH]
ldgr suggest commit --ticket ID [--target PATH]
ldgr migrate legacy-to-v1 --target PATH (--plan | --apply)
```

### 4.2 에이전트 쓰기 경로

```
ldgr ticket add   --json @-      # 새 ticket 첫 행 (status=open|in_progress)
ldgr ticket event --json @-      # 상태 전이·정정·노트 추가용 새 행
ldgr worklog add  --json @-
ldgr goal set     --json @- [--log]
```
...
- 새 CLI write에서는 `ticket`이 필수다. worklog는 audit-pass `done` ticket에 연결된 delivery record다.
- legacy/imported row는 호환을 위해 `ticket` 누락을 warning으로 다룰 수 있지만, 새 `ldgr worklog add`는 ticket-less row를 거부한다.
...
- `ticket add status=open`
  ...
  - 구현 ticket의 최소 provenance 형식:
    `archived=<path or none>; borrow=<path or none>; reference=<path or none>; new=<delta + why>`
  - 참고했지만 의도적으로 차용하지 않은 자산이 있으면 `not_borrowed=<why>`를 같은 `notes`에 남길 것
...
ldgr list [--prune]
ldgr goal show
ldgr next --ticket ID [--format text|json]
...
```json
{
  ...
  "warnings": [],
  "suggested_commands": ["ldgr ticket event --json @-"]
}
```
...
```
ldgr doctor [--target PATH] [--json]
ldgr insights [--target PATH] [--json]
```

`doctor`와 `insights`는 아직 shipped CLI가 아니다. 현재 shipped surface는
`verify`, `next`, `suggest`, `view`가 운영 진단을 나눠 맡는다. 후속
버전에서 별도 진단 명령을 추가할 경우 아래 의미를 따른다.
...
#### 7.2.2 Viewer ops surface follow-ups

After the first Control Tower release, the viewer should grow along four incremental surfaces:

- Drawer & owner surface: drawer shows audit notes, acceptance, decision, notes, provenance, claim/handoff fields, and reviewed_n backlinks; kanban cards show owner and stale-claim state; dashboard surfaces stale claims.
- Audit queue & lifecycle metrics: `/audit` page lists `audit_ready` tickets by priority and age; dashboard shows cycle time and audit latency; kanban cards mark stale in-progress and stale audit-ready work.
- Activity & verify surface: dashboard shows active agents from recent ledger rows; viewer exposes a read-only verify endpoint and dashboard widget with grouped fail/warn counts.
- Search & graph: global search covers ticket/worklog free text and paths; drawer shows blocker-chain mini diagrams; dashboard includes path heatmap. These are design-heavy and should follow a separate UI review.
...
- marker:
  ...
  # <<< LDGR_HOOK_END <<<
  ```
- 안전 백업: `.git/hooks/pre-commit.ldgr.bak` (uninstall에서 사용).
...
RTK의 `@RTK.md` 패턴처럼 긴 mutable prose를 각 instruction 파일에 직접 주입하지 않는다. 기본은 repo 안 ledger-owned instruction 파일을 만들고, `AGENTS.md`/`CLAUDE.md`에는 작은 marker 포인터만 둔다. 실제 강제는 prose가 아니라 `ldgr verify`, `next`, `suggest`, hooks가 담당한다.
...
- Claude/Codex 모두 기본은 작은 Markdown pointer다.
  ```
  <!-- LDGR_START -->
  See [`ledger/instructions/CLAUDE.ldgr.md`](ledger/instructions/CLAUDE.ldgr.md) for the full ldgr operating guide.
  <!-- LDGR_END -->
  ```
- Codex/other agents는 include 지원이 환경마다 다를 수 있으므로 기본은 minimal pointer다.
  ```
  <!-- LDGR_START -->
  See [`ledger/instructions/AGENTS.ldgr.md`](ledger/instructions/AGENTS.ldgr.md) for the full ldgr operating guide.
  <!-- LDGR_END -->
  ```
- inline/symlink mode는 후순위다. 파일 시스템 정책과 agent include semantics가 프로젝트마다 달라 기본값으로 쓰지 않는다.
- `ledger/instructions/*.ldgr.md` 파일은 CLI 호출 방법, append-only 규칙, claim/handoff 규칙, 검증 명령만 담는다. 프로젝트별 운영 지침은 기존 AGENTS/CLAUDE 본문에 남긴다.
- pointer block이 이미 존재하면 보존한다. `ledger/instructions/*.ldgr.md` 본문은 `instructions install` 때 현재 바이너리 내장 템플릿으로 갱신한다.
- `instructions uninstall`로 제거 가능하다. `--keep-bodies`는 pointer만 제거하고 `ledger/instructions/*.ldgr.md` 파일은 남긴다.
...
ldgr/
  ...
  templates/   # init 시드: goal.json, config.json, AGENTS_BLOCK.md
               # instruction bodies: AGENTS.ldgr.md, CLAUDE.ldgr.md
...
```
ldgr import legacy --target PATH --plan
ldgr import legacy --target PATH --apply [--archive-originals]
```
...
5. `import legacy --plan` / `--apply` (멱등 재빌드, 백업, parse-error 보존)
6. `view` (다중 프로젝트 대시보드, embed.FS; ready/blocker/stale/coverage 계산은 viewer aggregate에서 수행)
7. `hooks install` / `uninstall`, `instructions install` / `uninstall`, `next`, `suggest` (idempotent pointer mode + state-aware LLM guidance)
8. `migrate v1-to-v1 --plan` / `--apply` (명시적 breaking schema rewrite)
...
- `doctor` / `insights` 독립 CLI — 후속. 현재는 `verify`, `next`, `suggest`, `view`가 역할을 나눠 수행한다.
- 자동 repair 적용 — 후속. 실제 적용 자동화는 별도 plan에서 다룬다.
- command taxonomy/alias 관리 — 현재는 worklog command presence와 evidence를 verify/view에서 다루는 수준으로 제한한다.
...
| 동시성 | `ledger/.lock` + `~/.ldgr/registry.lock` (O_EXCL + 30s stale) |
...
| 운영 진단 | 현재 `verify` + `next` + `suggest` + `view`; 독립 `doctor`/`insights` CLI는 후속 |
| 완료 증거 | `ticket.acceptance` + `worklog.commands`/`worklog.evidence` |
| Hook | marker 기반 idempotent, 기존 보존 |
| 지시문 파일 | ledger-owned instruction file + small Markdown marker pointer |
